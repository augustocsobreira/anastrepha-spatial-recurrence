...
On Google Colab, open `spatial_recurrence_analysis.ipynb`, upload `data/trap_captures_long.csv` and `data/study_area_map.png` to `/content` through the file panel and run all cells; outputs are written to `/content/output`. If the map is not uploaded, the script reports it and skips Figure 1, which is produced in QGIS and not generated by the code; all other outputs are unaffected.
